...
- **Chomping doors:** vertical door pairs open/close on a ~3.7 s cycle (~0.9 s per phase) (all doors on a level in lockstep). Touching the jaws while drawn = a hit exactly like a knight's sword (white→gray, gray→life lost). Cross while the doorway is blank.
