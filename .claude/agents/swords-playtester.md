...
- Movement: WASD/arrows (8-way), `b` = back up without turning, `f` = descend stairs, `r` = read scroll. **Items are picked up by standing ON the tile with the disc released and pressing ENTER (or Space)** — walking over does nothing (ROM-accurate). Pickup locks movement for 21 frames. You carry at most 6 treasures; press ENTER at the level-1 start (the treasure chest) to store them for points (50/100/150/200 per treasure by level found; +1 reincarnation per 300 points). **Keypad `0` shows the status screen** (~4 s). When you lose a life the Prince falls in place and **revives only after you release the disc** — holding a direction keeps him down. Player speed 0.25 px/frame at a fixed 60 Hz (15 px/s — traversal is deliberately slow).
...
- `g.nearestEnemy()`, `g.nearestItem()`, `g.sorcerers()`, `g.stairs()`, `g.entry()`, `g.serpent()`, `g.crown()` — torus-aware distances.
- **Item pickup radius is small (<6 px)** — walking "near" an item misses it; steer to its exact coordinates before concluding pickup is broken.
...
1. **Honest play first** (feel + early balance): play levels 0–1 with real `hold()` input only — explore via `rowMap`, find the key, fight knights, reach and use the stairs. Note deaths, confusion, time taken.
...
3. **Probe edges**: torus wrap crossings mid-fight, stairs without key, dying with 1 reincarnation left, crown-before-Serpent, reading scrolls, back-up key (b) while a faster knight closes in.
